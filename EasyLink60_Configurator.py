...
def INTRO():
    ...
    print("         [EasyLink60 Configurator Software]         ")
...
#=======================[ADD PASSWORD]==================
    ...
    filedata = filedata.replace('ETHMAC', MAC)
...
def getNextMac():
    ...
    for i in range(workbook['MAC'].size):
        if(workbook['Serial Number (used)'].isnull().iloc[i]):
            nextAvailable = workbook['MAC'].iloc[i]
            sheet["B" + str(i+2)] = serialNumber
            found = i
            break
